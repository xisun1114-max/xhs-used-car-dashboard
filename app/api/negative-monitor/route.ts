...
export async function GET() {
  try {
    ...
    const [tasks, meta, owners, history] = await Promise.all([
      ...
      db.prepare("SELECT * FROM negative_monitor_history ORDER BY id DESC LIMIT 1000").all(),
    ]);
    ...
    return Response.json({ ...metadata, tasks: tasks.results, history: history.results, owners: (owners.results as Array<{owner:string}>).map((row) => row.owner) });
  } catch (error) {
